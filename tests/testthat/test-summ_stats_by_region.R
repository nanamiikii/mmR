...
test_that("summary statistics by mode (summarize_stats_by)", {
  ...
  expect_equal(tab$mean[1] |> round(), 1499)
  expect_equal(tab$median[1] |> round(), 144)
  expect_equal(tab$stdev[1] |> round(), 9418)
...
test_that("summary statistics for a specified region (summarize_stats_by_region)", {
  ...
  expect_equal(tab$`_data`$country[1], "Seychelles")
  expect_equal(tab$`_data`$mean[1], 15,233.50)
  expect_equal(tab$`_data`$median[1], 16,017.50)
